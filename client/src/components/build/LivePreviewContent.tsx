...
export const LivePreviewContent = ({ 
  device, 
  tickets = [], 
  jobStatus,
  errorMessage 
}: LivePreviewContentProps) => {
  // Calculate if we should show progress or preview
  const hasTickets = tickets && tickets.length > 0;
  const workTickets = hasTickets ? tickets.filter(t => t.type !== 'epic') : [];
  const completedTickets = workTickets.filter(t => t.status === 'done' || t.status === 'completed').length;
  const isBuilding = hasTickets && completedTickets < workTickets.length;
  const isFailed = jobStatus === 'failed';
  const showProgress = isBuilding || !hasTickets || isFailed;

  return (
    <div className={cn(
      "bg-background-elevated rounded-lg border border-border overflow-hidden flex flex-col relative",
      device === "mobile" 
        ? "w-[375px] h-[667px]" // iPhone-like dimensions (9:16 aspect ratio: 375x667)
        : "w-full aspect-video"
    )}>
      {/* Browser Controls */}
      <div className="flex items-center gap-2 p-2 bg-background-overlay border-b border-border z-10">
